...
void calBitmap(float **imageAvg)
{
    ...
    for(avgX = 0; avgX < width / BLOCK_WIDTH; avgX++)
    {
        for(avgY = 0; avgY < width / BLOCK_WIDTH; avgY++)
        {
            ...
            {
                avgHigh[avgX][avgY] = imageAvg[avgX][avgY] + sigma * sqrt((BLOCK_NUM - countHigher) / countHigher);
                avgLow[avgX][avgY] = imageAvg[avgX][avgY] - sigma * sqrt(countHigher / (BLOCK_NUM - countHigher));
            }
        }
    }
}
